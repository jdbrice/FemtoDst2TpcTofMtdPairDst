...
public:
	...
	// Compute the recentered value based on ideal mean and sigma
	double zb( FemtoTrackProxy &_proxy, string center ){
		LOG_SCOPE_FUNCTION(9);
		LOG_F( 9, "_proxy._track = %p", _proxy._track );
	...
	// Compute the Calibrated recentered value using the fit mean and sigma
	double zbCorr( FemtoTrackProxy &_proxy, string plc ){
		LOG_SCOPE_FUNCTION(9);
		double zbr = zb( _proxy, "mu" ); // !!not a typo - even pi should be computed wrt mu centered, since thats what the fits use.
		LOG_F(9, "testq");
		double p = _proxy._track->mPt * cosh( _proxy._track->mEta );
		int pBin = momentumBins.findBin( p );

		if ( 0 == parameters.count( plc + "_lambda_vs_p" ) ||
			 0 == parameters.count( plc + "_sigma_vs_p" ) ||
			nullptr == parameters[ plc + "_lambda_vs_p" ] || 
			nullptr == parameters[ plc + "_sigma_vs_p" ]  ){
			LOG_F( ERROR, "parameter histograms are NULL" );
			return -999;  
		}

		double lambda = parameters[ plc + "_lambda_vs_p" ]->GetBinContent( pBin+1 );
		double sigma  = parameters[ plc + "_sigma_vs_p" ]->GetBinContent( pBin+1 );

		zbr = (zbr - lambda) / sigma;
		return zbr;
	}
	...
	bool pass( FemtoTrackProxy &_proxy ){
		...
		double zbMu = zbCorr( _proxy, "mu" );
		double zbPi = zbCorr( _proxy, "pi" );

		// double avgP     = momentumBins.bins[ pBin ] + momentumBins.binWidth( pBin ) / 2.0;
		// double zbMu     = zbUtil.nlTof( "mu", _proxy._btofPid->beta(), p, avgP );
		// double zbPi     = zbMu;//zbUtil.nlTof( "pi", _proxy._btofPid->beta(), p, avgP );
		...
		// reject if outside of nSigmaMu range
		if ( !range[ "nSigmaMu" ].inInclusiveRange( zbMu ) ) return false;
		// reject if inside of nSigmaPi range
